...
``aiaccel-job`` is a small CLI that renders templated job scripts and either executes
them locally or submits them to PBS / SGE clusters. The workflow mirrors
``aiaccel.config``: compose a YAML file first, then select a backend (``local``,
``pbs``, or ``sge``) together with a workload type such as ``cpu``/``gpu``/``mpi``.
...
``aiaccel-job`` behaves like a regular CLI: pick a backend (``local``, ``pbs``, or
``sge``), choose the mode that matches your workload, provide a log filename, and pass
the command you want to run. Configuration files supply the templates that the CLI
expands, but the invocation style stays the same across environments. The implementation
focuses on loading the selected config, formatting the job snippet, and dispatching it
through the chosen backend with minimal ceremony.
...
The parser generated by :func:`aiaccel.job.apps.prepare_argument_parser` defines:

- Global options shared by every backend: ``--config`` (YAML path), ``--print_config``
  (dump and exit), and ``--walltime`` (overrides the config default for schedulers).
- Positional arguments: ``LOG_FILENAME`` determines where logs/script are written and
  ``COMMAND`` captures the payload.
- Mode-specific flags summarized below:
...
.. list-table::
    ...
    - - Mode
      ...
      - Description
    - - ``cpu`` / ``gpu``
      - ``--n_tasks``, ``--n_tasks_per_proc``, ``--n_procs``
      - Total task count (switches to ``*-array``), tasks per process (default from
        ``config['*-array']``), and parallel processes per array index.
    - - ``mpi``
      - ``--n_procs``, ``--n_nodes``
      - Required process count passed to ``mpirun`` plus optional node count (falls back
        to config when omitted).
    - - ``train``
      - ``--n_gpus``
      - Overrides the number of GPUs used by the training template.
...
.. list-table::
    ...
    - - Section
      - Purpose
    - - Global metadata
      - ``walltime``, ``script_prologue``, and other fields executed for every job.
    - - Mode sections
      - ``cpu``, ``cpu-array``, ``gpu``, ``mpi``, ``train``. Each provides ``job``
        templates and optional scheduler arguments.
    - - Scheduler adapters
      - Keys such as ``qsub`` / ``qsub_args`` that define how PBS / SGE submit the job.
    - - Format placeholders
      - ``{command}`` expands to the CLI payload, ``{args.*}`` exposes parsed arguments
        like ``n_tasks`` / ``n_procs`` / ``log_filename``.
...
.. code-block:: yaml
    ...
    gpu:
        job: "CUDA_VISIBLE_DEVICES=0 {command}"
...
- ``walltime`` propagates to schedulers that require a time limit. Locally it only
