...
class MnasNetBaseConv(NASModule):
    ...
    def build(self, config: dict[str, Any], **kwargs) -> None:
        ...
        max_in_ch, max_out_ch, self.stride, kernel_size_list, max_se_ratio = self.get_hyperparameters(**kwargs)
        self.skip_op = SkipOperation(self.stride)

        for k_size in kernel_size_list:
            self.candidate_ops.append(self.get_op(max_in_ch, max_out_ch, k_size, self.stride, max_se_ratio))
    ...
    def select_active_op(self, structure_info: MnasNetStructureInfo, prev_filter_size: int, **kwargs) -> None:
        ...
        Args:
            ...
            prev_filter_size (int): The filter size of the previous LAYER.
        """
...
class MnasNetLayer(NASModule):
    ...
    def build(self, config: dict[str, Any], **kwargs) -> None:
        """Creates supernet based on config file.
...
class MnasNetLayerStack(NASModule):
    ...
    def build(self, config: dict[str, Any], **kwargs) -> None:
        ...
        Args:
            config (dict): A dict type config that can be created using util function.
            layer_num (int): The number of layer stacks.
        """
...
class MnasNetBlock(NASModule):
    ...
    def __init__(self, name: str) -> None:
        ...
        self.layer_stack_num_list = []  # set at build
    ...
    def select_active_op(self, structure_info: MnasNetStructureInfo, prev_filter_size: int, **kwargs) -> None:
        """Sets subnet based on structure_info.
...
class MnasNetSearchSpace(NASModule):
    ...
    def build(self, config: dict[str, Any], **kwargs) -> None:
        ...
        if self.is_search:
            self.classifier = nn.Linear(self.last_ch, 200, bias=False)
        else:
            self.classifier = nn.Linear(self.last_ch, self.n_classes, bias=False)
    ...
    def select_active_op(self, structure_info: MnasNetStructureInfo, **kwargs) -> None:
        ...
        prev_filter_size = self.first_conv_filter_size
        # recursively setting
        for block in self.blocks:
    ...
    def enumerate_categorical_variables(self) -> dict[str, Any]:
        ...
        Returns:
            dict[str, Any]: A dict object of categorical variable.
        """
    ...
    def print_active_op(self, log_dir=None) -> None:
    ...
    def get_param_num_list(self) -> list[Any]:
        ...
        Returns:
            list[Any]: [[first_conv params], [Block0 param list], [Block1 param list], ...,
        [classifier params]]
        """
        params_list = []
