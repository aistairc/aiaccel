...
def scan_optuna_studies(runs_dir: Path) -> list[tuple[int, dict[str, float], dict[str, float]]]:
    ...
    # Preferred layout generated by modelbridge:
    # runs/<phase>/{macro,micro}/<run_id>/optuna.db
    for phase_path in sorted(runs_dir.iterdir()):
        if not phase_path.is_dir():
            continue

        macro_root = phase_path / "macro"
        micro_root = phase_path / "micro"
        if not macro_root.is_dir() or not micro_root.is_dir():
            continue

        macro_runs = {
            run_dir.name: run_dir / "optuna.db"
            for run_dir in sorted(macro_root.iterdir())
            if run_dir.is_dir() and (run_dir / "optuna.db").exists()
        }
        micro_runs = {
            run_dir.name: run_dir / "optuna.db"
            for run_dir in sorted(micro_root.iterdir())
            if run_dir.is_dir() and (run_dir / "optuna.db").exists()
        }

        for run_name in sorted(set(macro_runs) & set(micro_runs), key=int):
            try:
                run_idx = int(run_name)
            except ValueError:
                continue

            macro_best = _load_best_param(macro_runs[run_name])
            micro_best = _load_best_param(micro_runs[run_name])
