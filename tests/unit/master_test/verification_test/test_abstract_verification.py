...
class TestAbstractVerification(BaseTest):
    ...
    def test_verify(self, setup_hp_finished, work_dir):
        ...
        for i in range(10):
            verification.storage.result.set_any_trial_objective(trial_id=i, objective=(i * 1.0))
            verification.storage.trial.set_any_trial_state(trial_id=i, state="finished")
            verification.storage.hp.set_any_trial_params(
                trial_id=i, params=[{"parameter_name": f"x{j+1}", "value": 0.0, "type": "float"} for j in range(2)]
            )
    ...
    def test_make_verification(self, setup_hp_finished, work_dir):
        options = {"config": self.config_json, "resume": None, "clean": False, "fs": False, "process_name": "master"}
        ...
        for i in range(10):
            ...
            for j in range(2):
                verification.storage.hp.set_any_trial_param(
                    trial_id=i, param_name=f"x{j+1}", param_value=0.0, param_type="float"
                )
    ...
    def test_save(self, work_dir):
        ...
        for i in range(1):
            ...
            for j in range(2):
                verification.storage.hp.set_any_trial_param(
                    trial_id=i, param_name=f"x{j}", param_value=0.0, param_type="float"
                )

        verification.verify()
        path = work_dir.joinpath(aiaccel.dict_verification, f"1.{aiaccel.extension_verification}")
