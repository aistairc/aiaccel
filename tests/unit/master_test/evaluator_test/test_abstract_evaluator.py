from aiaccel.common import dict_hp_finished, dict_result, extension_hp, file_final_result
...
class TestAbstractEvaluator(BaseTest):
    ...
    def test_print(self, clean_work_dir, setup_hp_finished, work_dir):
        evaluator = AbstractEvaluator(self.load_config_for_test(self.configs["config.json"]))
        setup_hp_finished(1)
        evaluator.hp_result = work_dir.joinpath(dict_hp_finished, f"001.{extension_hp}")
