...
class TestAbstractScheduler(BaseTest):
    ...
    def test_change_state_finished_trials(self, setup_hp_running, setup_result, database_remove):
        options = {"config": self.config_json, "resume": None, "clean": False, "fs": False, "process_name": "scheduler"}
        database_remove()
    ...
    def test_get_stats(self, database_remove):
        database_remove()
        options = {"config": self.config_json, "resume": None, "clean": False, "fs": False, "process_name": "scheduler"}
        scheduler = AbstractScheduler(options)
        scheduler.print_dict_state()
    ...
    def test_start_job(self, setup_hp_ready, database_remove):
        database_remove()
        options = {"config": self.config_json, "resume": None, "clean": False, "fs": False, "process_name": "scheduler"}
        scheduler = AbstractScheduler(options)
        scheduler.print_dict_state()
    ...
    def test_update_resource(self, database_remove):
        database_remove()
        options = {"config": self.config_json, "resume": None, "clean": False, "fs": False, "process_name": "scheduler"}
        scheduler = AbstractScheduler(options)
        scheduler.print_dict_state()
    ...
    def test_pre_process(self, setup_hp_running, setup_result, database_remove):
        database_remove()
        options = {"config": self.config_json, "resume": None, "clean": False, "fs": False, "process_name": "scheduler"}
        scheduler = AbstractScheduler(options)
        scheduler.print_dict_state()
    ...
    def test_inner_loop_main_process(self, clean_work_dir, config_json, setup_hp_ready, database_remove):
        database_remove()
        options = {"config": self.config_json, "resume": None, "clean": False, "fs": False, "process_name": "scheduler"}
        scheduler = AbstractScheduler(options)
        scheduler.pre_process()
    ...
    def test_serialize(self, clean_work_dir, config_json, database_remove):
        database_remove()
        options = {"config": self.config_json, "resume": None, "clean": False, "fs": False, "process_name": "scheduler"}
        scheduler = AbstractScheduler(options)
    ...
    def test_deserialize(self, clean_work_dir, config_json, database_remove):
        database_remove()
        options = {"config": self.config_json, "resume": None, "clean": False, "fs": False, "process_name": "scheduler"}
        scheduler = AbstractScheduler(options)
        scheduler.storage.trial.set_any_trial_state(trial_id=0, state="finished")
    ...
    def test_check_error(self, config_json, database_remove):
        database_remove()
        options = {"config": self.config_json, "resume": None, "clean": False, "process_name": "scheduler"}
        scheduler = AbstractScheduler(options)
        assert scheduler.check_error() is True

        jobstates = [{"trial_id": 0, "jobstate": "failure"}]
