...
def test_generate_grid_points(grid_load_test_config):
    ...
    int_p = HyperParameter(
        {"name": "x4", "type": "uniform_int", "lower": 1, "upper": 10, "log": False, "step": 1, "base": 10}
    )

    assert generate_grid_points(int_p, config)

    cat_p = HyperParameter(
        {"name": "x3", "type": "categorical", "choices": ["red", "green", "blue"], "log": False, "step": 1, "base": 10}
    )
...
class TestGridOptimizer(BaseTest):
    ...
    def test_get_parameter_index(self, create_tmp_config):
        ...
        max_index = functools.reduce(lambda x, y: x * y, [len(p["parameters"]) for p in optimizer.ready_params])
