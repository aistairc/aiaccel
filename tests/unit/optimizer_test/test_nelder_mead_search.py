...
class TestNelderMeadOptimizer(BaseTest):
    ...
    def test_generate_parameter(self, load_test_config_org, setup_result):
        self.optimizer.pre_process()
        # config = load_test_config()
        config = load_test_config_org()
        self.optimizer.params = load_parameter(config.get("optimize", "parameters_for_TestNelderMead"))
    ...
    def test_generate_parameter2(self, load_test_config_org, setup_result):
        self.optimizer.pre_process()
        config = load_test_config_org()
        self.optimizer.params = load_parameter(config.get("optimize", "parameters_for_TestNelderMeadSearch"))
        rng = np.random.RandomState(0)
        self.optimizer.nelder_mead = NelderMead(self.optimizer.params.get_parameter_list(), rng=rng)
        # params = self.optimizer.nelder_mead.get_ready_parameters()
        # params = self.optimizer.get_ready_parameters()
    ...
    def test_update_ready_parameter_name(self, load_test_config_org):
        self.optimizer.pre_process()
        config = load_test_config_org()
        self.optimizer.params = load_parameter(config.get("optimize", "parameters_for_TestNelderMead"))
        rng = np.random.RandomState(0)
    ...
    def test_get_ready_parameters(self, load_test_config_org):
        self.optimizer.pre_process()
        config = load_test_config_org()
        self.optimizer.params = load_parameter(config.get("optimize", "parameters_for_TestNelderMead"))
        rng = np.random.RandomState(0)
