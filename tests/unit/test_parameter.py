...
class TestParameter(BaseTest):
    def test_get_best_parameters(self, work_dir, clean_work_dir):
        clean_work_dir()

        objective_y_index = 0
        files = list(work_dir.joinpath(dict_result).glob("*.yml"))
        best, best_file = get_best_parameter(files, goal_maximize, objective_y_index, work_dir.joinpath(dict_lock))
        assert best is None
        assert best_file is None

        results = [120, 101.0, np.float64(140.0)]
        ...
        best, best_file = get_best_parameter(files, goal_maximize, objective_y_index, work_dir.joinpath(dict_lock))
        assert best == 140.0
        best, best_file = get_best_parameter(files, goal_minimize, objective_y_index, work_dir.joinpath(dict_lock))
        assert best == 101.0
        try:
            _, _ = get_best_parameter(files, "invalid_goal", objective_y_index, work_dir.joinpath(dict_lock))
            assert False
        except ValueError:
            assert True
    ...
    def test_sample(self):
        ...
        json_string = [
            {"name": "a", "type": "uniform_int", "lower": 0, "upper": 10},
            {"name": "b", "type": "uniform_float", "lower": 0.0, "upper": 10.0},
            {"name": "c", "type": "categorical", "choices": ["red", "green", "blue"]},
            {"name": "d", "type": "ordinal", "sequence": ["10", "20", "30"]},
        ]
