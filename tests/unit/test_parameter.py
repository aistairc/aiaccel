...
class TestParameter(BaseTest):
    def test_get_best_parameters(self, work_dir, clean_work_dir):
        ...
        best, best_file = get_best_parameter(files, aiaccel.goal_maximize, work_dir.joinpath(aiaccel.dict_lock))
        assert best is None
        ...
        best, best_file = get_best_parameter(files, aiaccel.goal_maximize, work_dir.joinpath(aiaccel.dict_lock))
        assert best == 140.0
        best, best_file = get_best_parameter(files, aiaccel.goal_minimize, work_dir.joinpath(aiaccel.dict_lock))
        assert best == 101.0
        try:
            _, _ = get_best_parameter(files, "invalid_goal", work_dir.joinpath(aiaccel.dict_lock))
            assert False
        except ValueError:
            assert True
