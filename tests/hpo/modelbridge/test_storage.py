...
def test_pairs_from_paths(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    results = collect._pairs_from_paths("demo", "train", [macro_db, micro_db])
...
def test_pairs_from_layout_scan(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(collect, "_load_best_params", fake_load_best_params)
    results = collect._pairs_from_paths("demo", "train", list((scenario_output / "runs" / "train").rglob("optuna.db")))
    assert results == [(0, {"x": 1.0}, {"y": 2.0})]
...
def test_pairs_from_explicit_pairs(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    def fake_load_best_params(path: Path, _study_name: str | None) -> dict[str, float] | None:
        if path == macro_db:
            return {"x": 1.0}
        if path == micro_db:
            return {"y": 2.0}
    ...
    monkeypatch.setattr(collect, "_load_best_params", fake_load_best_params)
    results = collect._pairs_from_explicit_pairs([(macro_db, micro_db)])
    assert results == [(0, {"x": 1.0}, {"y": 2.0})]
